...
\sTODOinline{For any particular geminal category $G$, the free lexcategory with an internal geminal category extending $G$ should be an introspective theory, in the same way. The above is the case where $G$ is the initial geminal category.}
...
\begin{observation}
For any geminal category $\langle C'_1, C'_2; F'_1, F'_2 \rangle$, the operator $\Box_{C'_1} = \Gamma_{C'_2} \circ F'_1: C'_1 \to C'_1$ defined above has the strong \Loeb/ property (as defined at \magicref{LoebPropertyDefn}). This follows from the fact that, by \magicref{IntrospLoebAtEachAspect}, every aspect of every box operator on every introspective theory has the strong \Loeb/ property, and thus in particular, in the internal logic of the introspective theory $\langle \GLCatTheory, \underlying{K} \rangle$ of \magicref{GLCatTheoryIsIntrosp}, we have that $\Box_K$ has the strong \Loeb/ property (in more detail, the weak \Loeb/ property is satisfied with respect to the generic morphism of $\underlying{K}$ in its $\Mor(\underlying{K})$-aspect, thus establishing that it holds for all geminal categories with respect to all of their morphisms; uniqueness/the strong \Loeb/ property follows by a similar argument, or just by invoking \magicref{LoebPropertyLexUniqueness}).
\end{observation}
...
\sTODOinline{The following construction works even for some things that aren't geminal categories; in particular, if we define a K-category (a la the modal logic K) as a lexcategory with an internal lexcategory and a functor from the former to the global aspect of the latter (the first three of the six axioms of a geminal category), then every K-category has an underlying lexcategory-with-an-internal-lexcategory (even though K-categories do not have internal K-categories), and the right adjoint to this forgetful functor is given by the following construction. (Or maybe this needs to be a partial right adjoint defined only on those lex-categories-with-internal-K-categories?).

Actually, I think this doesn't work as a right adjoint just for K-category type things in general. I think it happens to work if all diagrams commute (K-posets), but otherwise... Well, let's figure this out again.

The fact that this same construction also gives the right adjoint for the forgetful functor from geminal categories to lexcategories-with-internal geminal categories is because of the following: The adjunction for K-categories tells us that, given a K-category k and a lexcategory-with-internal-lexcategory j, the lexcategory-with-internal-lexcategory maps from |k| to j are in correspondence with the K-category maps from k to Rj, where R is the right adjoint defined by the following construction. Now, the forgetful functor from geminal categories to K-categories is faithful; similarly, the forgetful functor from lexcategories-with-an-internal-geminal category to lexcategories-with-an-internal-lexcategory is faithful. The property that makes a K-category map into a geminal category map is preserving the $F'_1$ for $n \geq 1$, so to speak (it suffices just to have this for $n = 1$, but whatever). Similarly, the property that makes a lexcategory-with-an-internal-lexcategory map into a lexcategory-with-an-internal-geminal-category map is preserving the internal geminal category's $F'_n$ for $n \geq 0$ (which it will be less confusing to instead call the ambient structzure's $F'_n$ for $n \geq 1$; it suffices just to have this for $n \in \{1, 2\}$, but whatever). So we need to see that, when k is a geminal category and j is a lexcategory-with-internal-geminal-category, then a K-category map m from k to Rj preserves $F'_n$ for $n \geq 1$ iff the corresponding lexcategory-with-internal-lexcategory map from |k| to j preserves $F'_n$ for $n \geq 1$. That the former entails the latter is because the latter is the result of post-composing the counit |Rj| -> j to the former, and this counit preserves everything as it is essentially a projection. That the latter entails the former is because... The latter already tells us the first projection of the preservation condition holds. So we know $F'_n$ is sent to $(F'_n, something)$ and we need to ensure that the something is $F'_{n + 1}$. The something is equivalently the first projection applied to $F'_0$ applied to what $F'_n$ is sent to, which by the K-category map property is the first projection applied to what (F'0 of $F'_n$) is sent to, which is the first projection applied to what $F'_{n + 1}$ is sent to, which we already saw is $F'_{n + 1}$ as desired.

Note that the initial geminal category G on some generating objects maps into the following construction applied to Set-with-G-internal-to-it (mapping the generating objects arbitrarily), and in this way the theorems that G proves about its internal geminal category in fact apply to G as well. This relates to the reflection rule derivation given in https://sridharramesh.github.io/HowSridharThinks/reflectionrule/. The same applies to K-categories as above. Perhaps the categorical version of this is that, in the free geminal category on some set of generators, the map from G to the global aspect of its internal geminal category is an equivalence? And some similar fact for K-categories.}
\begin{construction}
